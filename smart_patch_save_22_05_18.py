...
def fisher_pixel(map_bd,map_bt,map_bs,last_values):
...
fisher_list,s_q_00,s_q_10,s_q_20,s_q_01,s_q_11,s_q_21,s_list,nnt_list,fisher_s_list,fisher_n_list=fisher_pixel(map_bd,map_bt,map_bs,last_values)
...
# print(np.shape(input_beta_zeroth_iteration))
print 'SQUALALA 0.5',sorted([np.sqrt(np.linalg.inv(fisher_list[x])[1][1])for x in pixel_list]) [-1]
a=delta_beta_matrix_making(P_d,P_t,P_s,input_beta_zeroth_iteration)
# print('delta=',a)

# comp.append(matrix_comparison(a,sigma))     UNCOMMENT !!!

print 'SQUALALA 1',sorted([np.sqrt(np.linalg.inv(fisher_list[x])[1][1])for x in pixel_list]) [-1]
# print('matrix comparison=',comp[0])
# print('input_beta_zeroth_iteration=',input_beta_zeroth_iteration)
map_b1,map_t1,map_s1=patch_map(input_beta_zeroth_iteration,P_d,P_t,P_s)
map_b=[]
map_t=[]
map_s=[]
map_b=map_b1[:]
...
i=0
hp.mollview(np.array(map_b),sub=(2,2,1),min=bounds[0][0],max=bounds[0][1])
...
plt.show()
# plt.close()
print 'SQUALALA 2',sorted([np.sqrt(np.linalg.inv(fisher_list[x])[1][1])for x in pixel_list]) [-1]
...
i=0
print 'SQUALALA 3',sorted([np.sqrt(np.linalg.inv(fisher_list[x])[1][1])for x in pixel_list]) [-1]
...
# print 'beta d map shape', np.shape(map_b)
...
print 'SQUALALA 4',sorted([np.sqrt(np.linalg.inv(fisher_list[x])[1][1])for x in pixel_list]) [-1]
...
print 'SQUALALA 5',sorted([np.sqrt(np.linalg.inv(fisher_list[x])[1][1])for x in pixel_list]) [-1]
...
x_axis_list=np.arange(bounds[1][0],50,0.1)
fun_list_plot=[]
gauss_list=[]
print 'map_b[pixel_list[150]]',map_t[pixel_list[150]]
for x in x_axis_list:
    fun_list_plot.append((-(1./2.)*fun[pixel_list[150]]([map_b[pixel_list[150]],x,map_s[pixel_list[150]]])))
    gauss_list.append((  -  np.log ( 1  /  np.sqrt(2*np.pi*np.linalg.inv (fisher_list[pixel_list[150]])[1][1]) )  *
                                (-(x-map_t[pixel_list[150]])*(x-map_t[pixel_list[150]])/(2*np.sqrt(np.linalg.inv (fisher_list[pixel_list[150]]))[1][1]) ) ))
min_fun=min(fun_list_plot)
for i in range(len(fun_list_plot)):
    fun_list_plot[i]=fun_list_plot[i]-min_fun
